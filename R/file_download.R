#' @rdname fileDownload
#' @export
fileDownloadUI <- function(id, lfc_thr=1, pval_thr=.05) {
...
#' Shiny module for data downloads
#'
#' Shiny module for data downloads
#'
#' This module presents a data download page. To configure the module, a
#' named list containing the data sets available for download should be
#' configured. 
#'
#' Each element of the list is one data set. The element corresponding to
#' the data set is, itself, a list containing the following elements:
#'  * title - Title fot the dataset
#'  * description - Detailed description of the data set
#'  * filename - the name of the file to be downloaded
#'  * data - either a data frame or a list of data frames
#'
#' If the parameter multiple is set to TRUE, then the datasets parameter is
#' expected to be a list of lists of data sets. This way, it is possible to
#' specify supersets of data sets.
#'
#' For example, if one data set is the raw counts from a transcriptomic
#' analysis, and the other corresponds to results of differential
#' expression analysis, the superset may contain such pairs of data sets
#' for several different experiments.
#'
#' Data set list can be a reactive value.
#' @param id module identifier (same as the one passed to volcanoUI)
#' @param datasets list of data sets (see Details)
#' @param multiple if TRUE, then the datasets is a list of datasets
#' @export

fileDownloadServer <- function(id, datasets, multiple=FALSE) {

  if(multiple) {
    message("volcanoServer: running in multi data set mode")
  } else {
    datasets <- list(default=datasets)
  }
