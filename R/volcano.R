# Internal helpers for normalizing volcano inputs across single/multi dataset modes.
...
volcanoServer <- function(id, cntr, annot=NULL, lfc_col="log2FoldChange", pval_col="padj", 
  ...
  moduleServer(id, function(input, output, session) {
    ...
    observeEvent(input$plot_hover, {
      .df <- dfvar()
      np <- nearPoints(.df, input$plot_hover, xvar="x", yvar="y")
      hover_genes(np[ , primary_id, drop=FALSE ])
    })

    observeEvent(input$plot_brush, {
      .df <- dfvar()
      np <- brushedPoints(.df, input$plot_brush, xvar="x", yvar="y")
      selected_genes(np)
    })

    observeEvent(input$plot_click, {
      .df <- dfvar()
      np <- nearPoints(.df, input$plot_click, xvar="x", yvar="y")
      selected_genes(np)
    })
